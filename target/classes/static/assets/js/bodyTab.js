...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                ulHtml += '<span class="layui-nav-more"><i class="iconfont icon-jiantou1"></i></span>';
                ulHtml += '</a>';
                ulHtml += '<dl class="layui-nav-child">';
                ulHtml += '<dt>'+data[i].name+'<button class="iconfont icon-rectangle710copy nav-child-close"></button></dt>';
                ...
                if(data[i].target == "_blank"){
                    ...
                    ulHtml += '<a data-url="'+data[i].url+'" title="'+data[i].name+'">';
                }
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ulHtml += '<i class="iconfont '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
	...
	Tab.prototype.render = function(dataStr) {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").append(_this.navBar(dataStr));
	...
	Tab.prototype.tabAdd = function(_this){
		...
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				title += '<i class="layui-unselect layui-tab-close iconfont icon-quxiao" data-id="'+tabIdIndex+'"></i>';
